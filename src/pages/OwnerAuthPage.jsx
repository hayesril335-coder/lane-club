...
export default function OwnerAuthPage({ onBack, onContinue, initialMode = 'signup' }) {
  ...
  return <main className="owner-auth-page">
    <section className="owner-form-panel">
      ...
      <div className="owner-form-wrap">
        {resettingPassword ? <PasswordResetForm emailLabel="Business email" busy={busy} message={message} onSubmit={sendReset} onBack={() => { setResettingPassword(false); setMessage('') }} /> : <>
          <div className="owner-tabs"><button className={mode === 'signup' ? 'active' : ''} onClick={() => { setMode('signup'); setMessage('') }}>Create owner account</button><button className={mode === 'login' ? 'active' : ''} onClick={() => { setMode('login'); setMessage('') }}>Log in</button></div>
          ...
          <form className="owner-form" onSubmit={submit}>
            ...
            {mode === 'login' && <button type="button" className="owner-forgot" onClick={() => { setResettingPassword(true); setMessage('') }}>Forgot password?</button>}
