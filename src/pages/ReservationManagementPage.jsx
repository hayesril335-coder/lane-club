...
export default function ReservationManagementPage({ bookings = [], alley, now = new Date() }) {
  ...
  const visibleHours = selectedDate === today ? hours.filter(hour => hour >= now.getHours()) : hours
  ...
  useEffect(() => {
    ...
    if (selectedDate === today && selectedHour < now.getHours()) setSelectedHour(now.getHours())
  }, [now, selectedDate, selectedHour, today])
  ...
  return <main className="reservations-page"><section className="reservations-content">
    ...
    <div className="schedule-controls"><label>Today or a future date<input type="date" min={today} value={selectedDate} onChange={event => { const date = event.target.value; setSelectedDate(date); setSelectedHour(date === today ? now.getHours() : 0); setSelected(null) }} /></label><strong>{dayBookings.length} reservation{dayBookings.length === 1 ? '' : 's'} on this day</strong></div>
    <div className="hour-strip" aria-label="Current and future hours of the day">{visibleHours.map(hour => <button key={hour} className={selectedHour === hour ? 'active' : ''} onClick={() => { setSelectedHour(hour); setSelected(null) }}>{formatHour(hour)}</button>)}</div>
