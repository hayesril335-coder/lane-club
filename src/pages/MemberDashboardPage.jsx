...
export default function MemberDashboardPage({ alley, now, onBack, onChangeAlley = onBack, onReserve, onReservations, onAccount, member }) {
  ...
  return <main className="member-dashboard">
    ...
    <section className="member-dash-shell">
      <header className="member-welcome"><div><p>YOUR MEMBER DASHBOARD</p><h1>Ready to <em>roll,</em> {member.name.split(' ')[0]}?</h1><span>{member.hasMembership ? `Your ${alley.name} membership is active.` : 'Choose an alley to start your membership.'}</span></div><button onClick={onChangeAlley}>Change Alley</button></header>
