...
export default function AlleySetupPage({ onBack, onComplete, initialAlley }) {
  ...
  const [lanes, setLanes] = useState(initialAlley?.lanes || 16)
  const [price, setPrice] = useState(initialAlley?.price || 20)
  const submit = (event) => {
    event.preventDefault()
    const fields = [...event.currentTarget.querySelectorAll('input, select')].map(field => field.value)
    setSaved(true)
    onComplete({ ...initialAlley, name: fields[0], phone: fields[1], website: fields[2], street: fields[3], city: fields[4], state: fields[5], lanes: Number(fields[6]), openingTime: fields[7], closingTime: fields[8], price: Number(fields[9]) })
  }
  ...
  return <main className="setup-page">
    ...
    <div className="setup-shell">
      ...
      <section className="setup-form-area"><form onSubmit={submit} className="setup-form">
        ...
        <div className="field-grid"><label className="wide">Bowling alley name<input required defaultValue={initialAlley?.name || ''} placeholder="e.g. Sunset Lanes" /></label><label>Phone number<input type="tel" required defaultValue={initialAlley?.phone || ''} placeholder="(555) 123-4567" /></label><label>Website <small>optional</small><input type="url" defaultValue={initialAlley?.website || ''} placeholder="https://youralley.com" /></label><label className="wide">Street address<input required defaultValue={initialAlley?.street || ''} placeholder="123 Main Street" autoComplete="street-address" /></label><label>City<input required defaultValue={initialAlley?.city || ''} placeholder="Los Angeles" autoComplete="address-level2" /></label><label>State<input required defaultValue={initialAlley?.state || ''} placeholder="CA" autoComplete="address-level1" /></label></div>
