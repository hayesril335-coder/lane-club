...
import { memberForAlley, membershipAlleyIds } from '../utils/memberships'
    ...
    <section className="settings-shell"><header><p>ACCOUNT SETTINGS</p><h1>Your <em>account.</em></h1><span>Manage your profile, memberships, and billing preferences.</span></header>
      <div className="settings-layout"><aside className="settings-nav">{tabs.map(item => <button key={item} className={section === item ? 'active' : ''} onClick={() => { setSection(item); setSaved('') }}>{item}</button>)}<hr /><button className="danger" onClick={() => setSection('Cancel membership')}>Cancel membership</button><button className="danger" onClick={onLogout}>Log out</button></aside>
        <section className="settings-panel">
          ...
          {section === 'Memberships' && <section><div className="settings-heading"><h2>Memberships</h2><p>Every bowling alley membership connected to your account.</p></div><div className="membership-settings-list">{memberships.length ? memberships.map(item => { const usedHours = memberForAlley(member, item).usedHours; const initials = item.name.split(' ').map(word => word[0]).join('').slice(0, 2).toUpperCase(); return <div className="membership-setting-card" key={item.id}><div className="setting-alley"><b>{initials}</b><div><strong>{item.name}</strong><span>{item.area} · Active member</span></div><i>Active</i></div><div className="setting-membership-info"><div><span>MONTHLY PRICE</span><strong>${item.price} / month</strong></div><div><span>7-DAY LANE TIME</span><strong>{usedHours} of 4 hours used</strong></div><div><span>STATUS</span><strong>Active</strong></div></div></div> }) : <p className="no-member-settings">You do not have an active membership yet. Use Search to join an alley.</p>}</div></section>}
