...
export default function MemberSettingsPage({ alley, alleys = [], onBack, onLogout, member, onUpdate }) {
    ...
    <section className="settings-shell"><header><p>ACCOUNT SETTINGS</p><h1>Your <em>account.</em></h1><span>Manage your profile, memberships, and billing preferences.</span></header>
      <div className="settings-layout"><aside className="settings-nav">{tabs.map(item => <button key={item} className={section === item ? 'active' : ''} onClick={() => { setSection(item); setSaved('') }}>{item}</button>)}<hr /><button className="danger" onClick={() => setSection('Cancel membership')}>Cancel membership</button><button className="danger" onClick={onLogout}>Log out</button></aside>
        <section className="settings-panel">
          {section === 'Profile' && <form onSubmit={saveProfile}><div className="settings-heading"><h2>Profile</h2><p>Update your personal information.</p></div><div className="profile-avatar"><b>{profile.name.split(' ').map(word => word[0]).join('').slice(0, 2).toUpperCase() || 'LC'}</b><div><strong>Profile information</strong><span>Your initials update from the name below.</span></div></div><div className="settings-fields"><label>Full name<input value={profile.name} onChange={event => setProfile(current => ({ ...current, name: event.target.value }))} required /></label><label>Email address<input type="email" value={profile.email} onChange={event => setProfile(current => ({ ...current, email: event.target.value }))} required /></label><label>Phone number<input type="tel" value={profile.phone} onChange={event => setProfile(current => ({ ...current, phone: event.target.value }))} placeholder="(555) 555-5555" /></label></div><button className="settings-save">Save changes</button></form>}
